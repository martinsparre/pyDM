...
SetLabels(20,20)
plt.annotate(r'$r_{-2}$', xy=(-0.35, 0.33), xytext=(-0.35, 0.45),fontsize=26,ha='center',arrowprops=dict(facecolor='grey', shrink=0.05))
plt.show()



r=scipy.sqrt( C.Snapshot.x*C.Snapshot.x +C.Snapshot.y*C.Snapshot.y  +C.Snapshot.z*C.Snapshot.z  )

plt.subplot(2,2,1)
plt.title(r'$0.05<r<0.3$')
IDs = scipy.where((r > 0.05 )*(r < 0.3 ))
plt.hist(C.Snapshot.vy[IDs], bins=25,label='vy',histtype='step')
plt.hist(C.Snapshot.vx[IDs], bins=25,label='vx',histtype='step')
plt.legend()

plt.subplot(2,2,2)
plt.title(r'$0.5<r<0.7$')
IDs = scipy.where((r > 0.5 )*(r < 0.7 ))
plt.hist(C.Snapshot.vy[IDs], bins=25,label='vy',histtype='step')
plt.hist(C.Snapshot.vx[IDs], bins=25,label='vx',histtype='step')
plt.legend()

plt.subplot(2,2,3)
plt.title(r'$2.0<r<3.0$')
IDs = scipy.where((r > 2.5 )*(r < 3.0 ))
plt.hist(C.Snapshot.vy[IDs], bins=25,label='vy',histtype='step')
plt.hist(C.Snapshot.vx[IDs], bins=25,label='vx',histtype='step')
plt.legend()
