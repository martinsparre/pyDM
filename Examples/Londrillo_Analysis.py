...
FileNames=['Hq1e6_rvir5e3_Beta0.15A_000','Hq1e6_rvir5e3_Beta0.15A_025']

FileNames = ['Data/Hq1e6_rvir5e3_Beta0.001_025',
'Data/Hq1e6_rvir5e3_Beta0.005.bin_025',
'Data/Hq1e6_rvir5e3_Beta0.01.bin_025',
'Data/Hq1e6_rvir5e3_Beta0.025.bin_025',
'Data/Hq1e6_rvir5e3_Beta0.04.bin_025',
'Data/Hq1e6_rvir5e3_Beta0.05.bin_025',
'Data/Hq1e6_rvir5e3_Beta0.075.bin_025',
'Data/Hq1e6_rvir5e3_Beta0.1.bin_025',
'Data/Hq1e6_rvir5e3_Beta0.2.bin_025','Hq1e6_rvir5e3_Beta0.15A_025','Hq1e6_rvir5e3_Beta0.15A_000']

#Color = ['black']*len(FileNames)
Color = ['red', 'blue', 'green', 'maroon','orange','black','pink','yellow','cyan']*2
Label = [r'$\Delta t = 250$']*len(FileNames)
Beta = [0.001,0.005,0.01,0.025,0.04,0.05,0.075,0.1,0.2,0.15,0.15]
...
#    plt.plot(log10(GridSph.R),GridSph.CumulativeMass  ,'-',color=Color[i],label=Label[i])        
    ...
    plt.subplot(1,3,1)
    plt.plot(log10(Beta[i]),LondrilloPsi,'o',color='blue')

    plt.subplot(1,3,2)
    plt.plot(log10(Beta[i]),10**(logrhalf-logr10),'o',color='blue')
    
    plt.subplot(1,3,3)
    plt.plot(log10(Beta[i]),2*scipy.sum(0.5*(A.Snapshot.vx**2+A.Snapshot.vy**2+A.Snapshot.vz**2)) / (0.5*scipy.sum(A.Snapshot.V)),'o',color='blue')    
