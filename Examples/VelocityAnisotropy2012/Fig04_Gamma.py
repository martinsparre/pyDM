...
ShowSim = 0*[False]+6*[True]+6*[False]
...
ax.text(-0., 0., r'I) $G$-variations', fontsize=20)
...
for i in range(len(Files)):
    ...
    if i==0:
        plt.plot(log10(GridSphA.R)+0.38,GridSphA.Gamma,'-'+mar[i],label=r'Spherical',color=col[i],lw=1,ms=7,mew=1)
    elif i==1:
        plt.plot(log10(GridSphA.R),GridSphA.Gamma,'-'+mar[i],label=r'Spherical',color=col[i],lw=1,ms=7,mew=1)
    else:
        plt.plot(log10(GridSphA.R)+0.6,GridSphA.Gamma,'-'+mar[i],label=r'Spherical',color=col[i],lw=1,ms=7,mew=1)
...
ax.text(-0., 0, r'II) Energy exchange', fontsize=20)
...
for i in range(len(Files)):
    ...
    GridSphA = A.CreateGridLogBins(NBins=50,Rmin=0.001,Rmax=50)

    plt.plot(log10(GridSphA.R[4:-1])+1.0,GridSphA.Gamma[4:-1],'-'+mar[i],label=r'Spherical',color=col[i],lw=1,ms=7,mew=1)
...
ax.text(-0., 0., r'III) Cold collapse', fontsize=20)
...
if ShowSim[2] == True:
    ...
    GridSph = A.CreateGridLogBins(NBins=100,Rmin=0.001)
    GridSphA = A.CreateGridLogBins(NBins=50,Rmin=0.01,Rmax=50)
    plt.plot(log10(GridSphA.R)+0.9,GridSphA.Gamma,'-o',label=r'Spherical',color='black',lw=1,ms=7,mew=1)
    ...
    GridSph = A.CreateGridLogBins(NBins=100,Rmin=0.001)
    GridSphA = A.CreateGridLogBins(NBins=50,Rmin=0.01,Rmax=50)
    plt.plot(log10(GridSphA.R)+0.9,GridSphA.Gamma,'-x',label=r'Spherical',color='black',lw=1,ms=7,mew=1)    
...
ax.text(-0., 0., r'IV) Major mergers', fontsize=20)
...
if ShowSim[3] == True: 
    for i in range(len(Files)):
        ...
        A.CenterParticleVelocities()
        GridSphA = A.CreateGridLogBins(NBins=50,Rmin=0.01,Rmax=50)
        plt.plot(log10(GridSphA.R)-Offset[i],GridSphA.Gamma,mar[i],label=r'Spherical',color=col[i],lw=1,ms=7,mew=1,zorder=Zorder[i])
...
ax.text(-0., -0., r'V) Unstable model', fontsize=20)
...
if ShowSim[4] == True:
    for i in range(len(Files)):
        ...
        GridSphA = A.CreateGridLogBins(NBins=50,Rmin=0.01,Rmax=50)
        plt.plot(log10(GridSphA.R)+Offset[i],GridSphA.Gamma,mar[i],label=Label[i],color=col[i],lw=2,ms=7,mew=1)

plt.xlim((-0.99,1))
plt.ylim((-4.99,0.99))

plt.legend(prop=dict(size=14), numpoints=2, ncol=1,frameon=True,loc=3)
#plt.annotate(r'$r_{-2}$', xy=(-0.15, 0.5), xytext=(-0.15, 0.65),fontsize=18,ha='center',arrowprops=dict(facecolor='grey', shrink=0.05))
#plt.grid()
plt.ylabel(r'$\gamma$',fontsize=24)
...
ax.text(-0., 0., 'VI) Substructure', fontsize=20)
...
if ShowSim[5] == True: 
    ...
    plt.plot(log10(GridSphA.R)+0.25,GridSphA.Gamma,'-o',label=r'IC',color='black',lw=2,ms=9,mew=2)
    ...
    plt.plot(log10(GridSphA.R)+0.25,GridSphA.Gamma,'-D',label=r'Final',color='red',lw=2,ms=9,mew=2)
